export type MultiClassNames = string & { __MultiClassNames: true }

export type ClassName<T = string> = T | false | void | null | 0 | ''

interface ICX {
  (...classNames: ClassName<MultiClassNames>[]): MultiClassNames
...
const cx: ICX = function cx() {
  const presentClassNames: (ClassName | ClassName<MultiClassNames>)[] = Array.prototype.slice
  ...
  return result.join(' ') as MultiClassNames
}
